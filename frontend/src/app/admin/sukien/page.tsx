...
export default function SuKienTablePage() {
  ...
  const fetchData = async () => {
    const url = new URL('http://localhost:10000/api/sukien/get/all')
  ...
  useEffect(() => {
    if (addModalOpen || editModalOpen) {
      fetch('http://localhost:10000/api/danhmucsukien/get/all?size=100', {
        credentials: 'include',
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const onSubmitAddForm = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:10000/api/sukien/add', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:10000/api/sukien/update/${editForm.maSuKien}`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
    ...
    <div>
      ...
      <div className="modal-buttons" style={{ marginTop: '20px' }}>
        <button
          ...
          onClick={async () => {
            try {
              const res = await fetch(`http://localhost:10000/api/sukien/cancel/${selectedCancel.maSuKien}`, {
                method: 'PUT',
                credentials: 'include',
              });
