...
export default function CauHoiPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:10000/api/cauhoi/get/all?${query}`, {
        credentials: 'include'
      });
      if (res.ok) {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:10000/api/cauhoi/answer/${selectedCauHoi.maCauHoi}`, {
        method: 'POST',
        credentials: 'include',
