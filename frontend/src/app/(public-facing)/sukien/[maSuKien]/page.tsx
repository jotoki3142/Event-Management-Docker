...
export default function EventDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:10000/api/sukien/get/${maSuKien}`, {
      credentials: 'include',
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:10000/api/danhgia/sukien/${maSuKien}/get/all`, {
      credentials: 'include',
    })
      .then((res) => res.json())
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://localhost:10000/api/danhgia/${event.maSuKien}/add`, {
        method: 'POST',
        headers: {
